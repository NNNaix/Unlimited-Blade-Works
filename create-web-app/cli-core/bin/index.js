...
try {
    process.title = 'create-web-app ' + Array.from(process.argv).slice(2).join(' ');
} catch (_) {
    // If an error happened above, use the most basic title.
    process.title = 'create-web-app';
}
...
if (semver.satisfies(major, supportedNodeVersion)) {
    console.error(
        ...
        '.\n' +
        chalk.yellow('创建 Web 应用需要 Node 版本为：' + chalk.yellowBright.bold(supportedNodeVersion) + ' ，请升级 Node 至脚手架支持的最低版本.') +
        '\n\n' +
...
const {init} = require('../lib/createWebApp.js');
